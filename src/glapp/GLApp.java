...
/**
 * Collection of functions to init and run an OpenGL app using LWJGL. <P>
 * Includes functions to handle: <BR> Setup display mode, keyboard, mouse,
 * handle events<BR> Run main loop of application <BR> Buffer allocation --
 * manage IntBuffer, ByteBuffer calls. <BR> OpenGL functions -- convert
 * screen/world coords, set modes, lights, etc. <BR> Utility functions -- load
 * images, convert pixels, getTimeInMillis, etc. <BR> <P> Has a main() function
 * to run as an application, though this class has only minimal placeholder
 * functionality. It is meant to be subclassed, and the subclass will define
 * setup() draw() mouseMove() functions, etc. <P> GLApp initializes the LWJGL
 * environment for OpenGL rendering, ie. creates a window, sets the display
 * mode, inits mouse and keyboard, then runs a loop. <P> Uses GLImage to load
 * and hold image pixels. <P> napier -at- potatoland -dot- org
 *
 * @see GLImage
 */
public class GLApp {
    ...
    public static int MaxFps = 80;                       // If Vsync is off, then set the target FPS
     ...
     * keyboard input. <P> Calls init(), handleEvents(), update() and draw().
     * <BR> handleEvents() calls: mouseMove(), mouseDown(), mouseUp(),
     * keyDown(), keyUp()
     */
    public void run() {
    ...
    /**
     * Initialize the Keyboard and Mouse. <P> Disable or hide the native cursor.
     * Set the initial cursor position. Get the timer resolution (ticks per
     * second).
     *
     * @see handleEvents()
     */
     ...
     * load textures, models, and create data structures used by the app.
     *
     * This function is called only once, when application starts. It is called
     * after initDisplay and initOpenGL(), so the OpenGL context is already
     * created.
     *
     * @see init()
     */
     ...
     * viewport to a sub-region of the screen. <P> This function is only valid
     ...
     * click outside the window area. <P> If the param is false, the hardware
     * cursor will behave normally. Use hideHardwareCursor() to show or hide the
     * hardware cursor.
     *
     ...
     * wanted to hide it. <P> If param is false, reset the cursor to the
     * default.
     *
     ...
     *
     * @param whichSetting the id number of the value to be returned (same
     * constants as for glGetInteger())
     */
     ...
     * format.<BR> Configure the texture to repeat in both directions and use
     * LINEAR for magnification. <P>
     *
     * @return the texture handle
     */
     ...
     * accordingly when creating the texture. <P> Configure the texture to
     * repeat in both directions and use LINEAR for magnification. <P> NOTE: I'm
     * having problems creating mipmaps when image pixel data is in GL_BGRA
     * format. Looks like GLU type param doesn't recognize
     ...
     * reversed on Mac. Have to stick with GL_RGBA byte order for now. <P>
     *
     * @return the texture handle
     */
     ...
     * getPowerOfTwoBiggerThan(800) will return 1024. <P>
     *
     ...
     * pixel position 100,100. <P> <B>ABOUT Ortho and Viewport:</B><br> Let's
     * say we're drawing in 2D and want to have a cinema proportioned viewport
     * (16x9), and want to bound our 2D rendering into that area ie.
     * <PRE>
     * ___________1024,576
     * |           |
     ...
     * <P> NOTE: if the viewport is the same size as the window (by default it
     * is), then setOrtho() will make the world coordinates exactly match the
     * screen pixel positions. This is convenient for mouse interaction, but be
     * warned: if you setViewport() to something other than fullscreen, then you
     * need to use getWorldCoordsAtScreen() to convert screen xy to world xy.
     * <P> Once Ortho is on, glTranslate() will take pixel coords as arguments,
     ...
     * setOrthoDimensions(). <P> By default ortho mode (setOrtho(),
     * setOrthoOn()) will set the world width and height to exactly match the
     * screen pixel width and height (it uses the viewport size, which is
     * typically the exact same size as the opengl window). This makes it easy
     * to map screen positions to the world space. Text positions, images and
     * mouse coordinates map pixel-for-pixel to the screen. <P> The drawback is
     * that the ortho world space will change size on different resolution
     * screens. In low-res screens the ortho world may be 800x600 while in a
     * hi-res screen it could be 1920x1200, which means that anything drawn in
     * ortho mode may shift on different screens. <P> setOrthoDimensions()
     * assigns a fixed size for ortho scenes, independent of the viewport size.
     * OpenGL will project the ortho world into the viewport just as it does
     * with a perspective projection. Things drawn in ortho will stay in the
     * right place on any resolution screen. <P> The drawback with this method
     * is that screen coordinates don't necessarily map exactly to the ortho
     * world. To translate a screen xy to the ortho world coordinates, use
     * getWorldCoordsAtScreen(x,y) just as with perspective worlds.
     *
     ...
     * (displayWidth,displayHeight). <P> NOTE: by default the window size,
     * viewport size and setOrtho() size are all the same, so in ortho mode
     * screen pixel positions exactly match to world coordinates. THIS IS NO
     * LONGER TRUE if you setViewport() to some other size. With a custom
     * viewport you need to use getWorldCoordsAtScreen() to convert screen xy to
     * world xy. <P>
     *
     ...
     * turn on blending, set color to white and turn off depth test. <P> call
     * pushAttribOverlay(), enable settings that you need, when done call
     * popAttrib()
     *
     * @see popAttrib()
     */
     ...
     * 'Ambient' colors (direct light and reflected light), and the
     * position.<BR>
     *
     * @param GLLightHandle
     ...
     * (range 0-1). <P> Sets diffuse material color to the surfaceColor and
     * ambient material color to surfaceColor/2. Based on the shiny value (0-1),
     * sets the specular property to a color between black (0) and white (1),
     * and sets the shininess property to a value between 0 and 127. <P>
     * Lighting must be enabled for material colors to take effect. <P>
     *
     ...
     * take effect.
     *
     * @param shininess: size of reflection (0 is matte, 127 is pinpoint
     * reflection)
     */
     ...
     * value to the given value @para alpha 0=transparent 1=opaque
     */
     ...
     * Sys.getTime(). <P> Called by run() at the beginning of each loop.
     *
     * @see run()
     ...
     * pixels.<BR> Can be used to create textures. <BR>
     *
     * @param imgFilename
     * @return
     */
     ...
     * after scene is drawn to place a cursor on top of scene. <P> NOTE: the
     * cursor is drawn in screen space, at an absolute screen pixel location
     * without regard for viewport (temporarily zets viewport to entire screen).
     * <P> See handleEvents() for cursorX cursorY and mouse motion handling. <P>
     * Example:
     * <PRE>
     *    int cursorTxtr;
     *
     ...
     * </PRE>
     *
     * @param cursorTextureHandle handle to texture containing 32x32 cursor
     * image
     */
     ...
     * after scene is drawn. Uses glPushAttrib() to preserve the current drawing
     * state. glPushAttrib() may slow performance down, so in your app you may
     * want to set the states yourself before calling drawCursor() and take the
     * push/pop out of here. <P> See handleEvents() for cursorX cursorY and
     * mouse motion handling. <P> Example:
     * <PRE>
     *    int cursorTxtr;
     *
     *    public void setup() {
     ...
     * so transparent areas will be respected. <P> NOTE: By default the viewport
     * is the same size as the window so this function will draw the image over
     * the entire window. If you setViewport() to a custom size the image will
     * be drawn into the custom viewport area. To insure that the image is drawn
     * truly full screen, call resetViewport() before drawImageFullScreen(). <P>
     *
     ...
     * material if any are active. <BR> @ee loadImage()
     */
     ...
     * <P> Reset the viewport and ortho mode to full screen (viewport may be
    ...
    public static void print(int x, int y, String msg, int set) {
        ...
        if (fontListBase == -1 || fontTextureHandle == -1) {
            ...
            if (!buildFont("res/images/font_tahoma.png", 12)) {
     ...
     * gl functions will operate on this buffer instead of the display. <P>
     * NOTE: the Pbuffer may be recreated if it was lost since last used. It's a
     * good idea to use:
     * <PRE>
     *         pbuff = selectPbuffer(pbuff);
     * </PRE> to hold onto the new Pbuffer reference if Pbuffer was recreated.
     *
     ...
     * have been rendered into the offscreen pbuffer. <P> WARNING: this function
     * hogs memory! Call java with more memory (java -Xms128m -Xmx128m) <P>
     *
     ...
     * function here for reference. <P> If the current buffer is the framebuffer
     * then this will work as a screen capture. Can also be used with the
     * PBuffer class to copy large images or textures that have been rendered
     * into the offscreen pbuffer. <P> WARNING: this function hogs memory! Call
     * java with more memory (java -Xms128m -Xmx128) <P>
     *
     * @see selectPbuffer(), selectDisplay()
     */
     ...
     * <P> To use, create a display list in setup():
     * <PRE>
     ...
     * @param values target integer array, must be same length as ByteBuffer
     * capacity/4
     */
    ...
    public static InputStream getInputStream(String filename) {
        ...
        if (in == null && rootClass != null) {
            ...
            try {
                ...
                msg("GLApp.getInputStream(): Can't find resource: " + ue);
            }
            ...
            if (in == null && appletBaseURL != null) {
                try {
                    ...
                    msg("GLApp.getInputStream(): Can't make applet base url: " + ue);
                }
     ...
     * name for use in screenshot filenames (see screenShot()). <P> To load
     * files from a jar we need to access a class in the root folder of the
     * installation. It's not good to use GLApp.class because that class is in
     * the glapp package folder, and the getResource() function will not find
     ...
     *      images (folder)
     *      sounds (folder)
     * </PRE> In this case setRootClass() will set the rootClass to MyGame. If
     * MyGame and subfolders are packaged in a jar file, then getInputStream()
     * should be able to do a rootClass.getResource("models/some_model.obj") and
     * correctly retrieve the file from the JAR. <P>
     *
     * @see getInputStream()
     */
     ...
     * using invoke() (similar to a callback function in C/C++). <P> NOTE:
     * method invocation is very fast for methods that take no parameters. If
     * the method takes parameters then invoking is much slower than calling the
     * function directly through code. For this reason and for simplicity I
